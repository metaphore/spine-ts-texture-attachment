...
module spine.custom {
  ...
  export class CustomSkeletonRenderer {
    ...
    constructor(context: spine.webgl.ManagedWebGLRenderingContext, twoColorTint: boolean = true) {
    ...
    draw(batcher: spine.webgl.PolygonBatcher, skeleton: Skeleton, slotRangeStart: number = -1, slotRangeEnd: number = -1) {
      ...
      for (let i = 0, n = drawOrder.length; i < n; i++) {
        ...
        let texture: spine.webgl.GLTexture = null;
        if (attachment instanceof RegionAttachment) {
          ...
          uvs = region.uvs;
          texture = <spine.webgl.GLTexture>(<TextureAtlasRegion>region.region.renderObject).texture;
          ...
          texture = <spine.webgl.GLTexture>(<TextureAtlasRegion>mesh.region.renderObject).texture;
          uvs = mesh.uvs;
          attachmentColor = mesh.color;
        } else if (attachment instanceof TextureAttachment) {

          let textureAttach = <TextureAttachment>(attachment);
          ...
          triangles = spine.webgl.SkeletonRenderer.QUAD_TRIANGLES;
          texture = <spine.webgl.GLTexture>(textureAttach.texture);
        ...
        if (texture != null) {
          ...
          if (slotBlendMode != blendMode) {
            ...
            batcher.setBlendMode(
              spine.webgl.WebGLBlendModeConverter.getSourceGLBlendMode(blendMode, premultipliedAlpha),
              spine.webgl.WebGLBlendModeConverter.getDestGLBlendMode(blendMode));
          }
